...
#include "../../include/core/profiling.h"
#include <SDL_image.h>
...
TextureData::TextureData(GLenum textureTarget, int width, int height, int numTextures, void** data, GLfloat* filters, GLenum* internalFormat, GLenum* format, bool clamp, GLenum* attachments)
{
...
void TextureData::InitTextures(void** data, GLfloat* filters, GLenum* internalFormat, GLenum* format, bool clamp)
{
...
Texture::Texture(const std::string& fileName, GLenum textureTarget, GLfloat filter, GLenum internalFormat, GLenum format, bool clamp, GLenum attachment)
{
	...
	{
		SDL_Surface* surface = IMG_Load(("./res/textures/" + fileName).c_str());
		int x = surface->w;
		int y = surface->h;
		...
		m_textureData = new TextureData(textureTarget, x, y, 1, &surface->pixels, &filter, &internalFormat, &format, clamp, &attachment);
