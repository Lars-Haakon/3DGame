...
public:
	...
	inline Matrix4<T> InitRotationFromVectors(const Vector3<T>& n, const Vector3<T>& u, const Vector3<T>& v)
	{
		(*this)[0][0] = v.GetX();   (*this)[1][0] = v.GetY();   (*this)[2][0] = v.GetZ();   (*this)[3][0] = T(0);
	...
	{
		const T flRight = zNear * tanf(fov / T(2));
		const T flLeft = -flRight;
		const T flTop = flRight / aspectRatio;
		const T flBottom = flLeft / aspectRatio;

		(*this)[0][0] = 2 * zNear / (flRight - flLeft); (*this)[1][0] = T(0);							(*this)[2][0] = (flRight + flLeft) / (flRight - flLeft);    (*this)[3][0] = T(0);
		(*this)[0][1] = T(0);							(*this)[1][1] = 2 * zNear / (flTop - flBottom); (*this)[2][1] = (flTop + flBottom) / (flTop - flBottom);    (*this)[3][1] = T(0);
		(*this)[0][2] = T(0);							(*this)[1][2] = T(0);							(*this)[2][2] = (-zFar - zNear) / (zFar - zNear);			(*this)[3][2] = -2 * zFar * zNear / (zFar - zNear);
...
public:
	...
	{
		Vector3f v = Vector3f(1.0f - 2.0f * (GetY()*GetY() + GetZ()*GetZ()), 2.0f * (GetX()*GetY() - GetW()*GetZ()), 2.0f * (GetX()*GetZ() + GetW()*GetY()));
		Vector3f u = Vector3f(2.0f * (GetX()*GetY() + GetW()*GetZ()), 1.0f - 2.0f * (GetX()*GetX() + GetZ()*GetZ()), 2.0f * (GetY()*GetZ() - GetW()*GetX()));
		Vector3f n = Vector3f(2.0f * (GetX() * GetZ() - GetW() * GetY()), 2.0f * (GetY() * GetZ() + GetW() * GetX()), 1.0f - 2.0f * (GetX() * GetX() + GetY() * GetY()));
		
		return Matrix4f().InitRotationFromVectors(n, u, v);
	}
